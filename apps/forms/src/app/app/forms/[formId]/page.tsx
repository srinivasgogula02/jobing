...
export default async function ResponsesPage({ params }: { params: Promise<{ formId: string }> }) {
  ...
  return <div className="app-layout">
    <aside className="app-sidebar"><a className="brand" href={`${process.env.NEXT_PUBLIC_JOBING_SITE_URL || "https://jobing.site"}/dashboard`} aria-label="Jobing AI dashboard"><span className="brand__jobing">Jobing</span><span className="brand__product">Forms</span></a><nav className="app-nav" aria-label="Forms navigation"><a href={`${process.env.NEXT_PUBLIC_JOBING_SITE_URL || "https://jobing.site"}/dashboard`}>Dashboard</a><Link href="/app" aria-current="page">Forms</Link><Link href="/app/activity">Activity</Link><Link href="/app/system">System</Link></nav></aside>
