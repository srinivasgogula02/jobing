...
export async function POST(request: Request, context: { params: Promise<{ endpoint: string }> }) {
  ...
  if (length > MAX_BYTES) return NextResponse.json({ error: { code: "request_too_large", message: "The submission is too large." } }, { status: 413 });
  const form = await getPublicForm(endpoint);
  if (!form) return NextResponse.json({ error: { code: "form_not_found", message: "This form is unavailable." } }, { status: 404 });
  ...
  if (!validated.success) return html(renderPublicForm({ definition: form.definition, endpointId: endpoint, action: canonical(endpoint), siteKey: process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY || "", submissionId: String(data.get("_submission_id") || randomUUID()), errors: validated.errors }), 422);
  const secret = process.env.SUBMISSION_IP_HASH_SECRET;
  if (!secret || Buffer.byteLength(secret) < 32) return NextResponse.json({ error: { code: "unavailable", message: "Forms is temporarily unavailable." } }, { status: 503 });
  const origin = originHeader === hostedOrigin ? null : originHeader;
  try {
    ...
    return NextResponse.json({ error: { code: status === 429 ? "rate_limited" : status === 403 ? "origin_not_allowed" : "submission_failed", message: status === 500 ? "The response could not be saved." : "This submission is not allowed." } }, { status });
  }
}
...
export async function OPTIONS(request: Request, context: { params: Promise<{ endpoint: string }> }) {
  ...
  if (!form || !origin || !(form.definition.settings?.allowedOrigins ?? []).includes(origin)) return new NextResponse(null, { status: 403 });
