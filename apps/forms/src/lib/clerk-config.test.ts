...
describe("Forms Clerk configuration", () => {
  it("redirects sign-in and sign-up completions to the fixed Forms app path", () => {
    expect(getFormsSignInRedirectProps("https://forms-preview.jobing.site", "production")).toEqual({
      mode: "redirect",
      forceRedirectUrl: "https://forms-preview.jobing.site/app",
      signUpForceRedirectUrl: "https://forms-preview.jobing.site/app",
    });
  });

  it("never accepts a path, query, credentials, or non-web scheme as the return origin", () => {
    expect(() => getFormsSignInRedirectProps("https://forms.jobing.site/elsewhere"))
      .toThrow("NEXT_PUBLIC_FORMS_SITE_URL");
    expect(() => getFormsSignInRedirectProps("https://user:pass@forms.jobing.site"))
      .toThrow("NEXT_PUBLIC_FORMS_SITE_URL");
  ...
  it("normalizes and deduplicates explicitly authorized origins", () => {
    expect(resolveClerkAuthorizedParties(
      "https://jobing.site/, https://forms.jobing.site,https://jobing.site",
      "production",
    )).toEqual(["https://jobing.site", "https://forms.jobing.site"]);
  });
