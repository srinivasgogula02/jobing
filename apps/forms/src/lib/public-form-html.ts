...
function control(field: FormDefinition["fields"][number], value?: unknown) {
  const attrs = `id="${escape(field.key)}" name="${escape(field.key)}"${field.required ? " required" : ""}${field.placeholder ? ` placeholder="${escape(field.placeholder)}"` : ""}`;
...
export function renderPublicForm(input: { definition: FormDefinition; endpointId: string; action: string; submissionId?: string; message?: string; formError?: string; errors?: Record<string,string>; values?: Record<string,unknown> }) {
  ...
  return `<!doctype html><html lang="en" data-font="${escape(theme.fontFamily)}" data-spacing="${escape(theme.spacing)}" data-button="${escape(theme.buttonStyle)}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escape(definition.title)} · Jobing Forms</title><link rel="stylesheet" href="/forms/public-form.css"><script src="/forms/public-form.js" defer></script></head><body style="${themeStyle}"><main><header><a href="https://jobing.site/forms" class="brand">Jobing <span>Forms</span></a><p class="eyebrow">Secure response</p><h1>${escape(definition.title)}</h1>${definition.description ? `<p class="lede">${escape(definition.description)}</p>` : ""}</header>${input.message ? `<section class="success" role="status"><strong>${escape(definition.confirmation.title)}</strong><p>${escape(input.message)}</p></section>` : `<form method="post" enctype="multipart/form-data" action="${escape(input.action)}" data-public-form>${input.formError ? `<section class="form-error" role="alert"><strong>We could not send your response.</strong><p>${escape(input.formError)}</p></section>` : ""}<input type="hidden" name="_submission_id" value="${escape(input.submissionId ?? "")}"><input type="hidden" name="_jobing_form_context" value="hosted"><label class="trap" aria-hidden="true">Leave this empty<input type="text" name="_gotcha" tabindex="-1" autocomplete="off"></label>${fields}<button type="submit" data-submit-button>Send response</button><p class="privacy">Your response is sent securely to the form owner.</p></form>`}</main></body></html>`;
}
