...
export const TELEMETRY_SERVICE = "pages-runtime";
...
/**
 * Last-line privacy filter for Sentry. Generated HTML, request data, and runtime
 * values never leave this service; stack frames remain available for debugging.
 */
export function scrubSentryEvent<T extends Sentry.Event>(event: T): T {
  ...
  event.message = event.message ? "Pages Runtime operational error" : undefined;
  ...
  for (const value of event.exception?.values ?? []) {
    value.value = "Pages Runtime operational error";
...
export function scrubSentryTransaction<T extends Sentry.Event>(event: T): T {
  ...
  event.transaction = "pages-runtime.request";
