      ...
      <div className="min-h-full bg-[#f7f8f4] px-4 py-6 sm:px-6 lg:px-10 lg:py-9">
        <div className="mx-auto max-w-6xl">
          <header className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
            ...
            <Link href="/pages" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#151914] px-5 font-semibold text-white hover:bg-[#293025] focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-[#719500]">
              <Plus size={17} /> Create a page
            </Link>
          </header>

          {pages.length === 0 ? (
            <section className="mt-7 grid min-h-[360px] place-items-center rounded-[22px] border border-dashed border-[#cbd1c6] bg-white p-7 text-center">
              <div className="max-w-md">
                <span className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-[#eaf8dc] text-[#4b7000]"><Globe2 size={25} /></span>
                ...
                <Link href="/pages" className="mt-6 inline-flex min-h-11 items-center justify-center rounded-xl bg-[#c1ff00] px-5 font-semibold text-[#151914]">Open page editor</Link>
              </div>
            </section>
          ) : (
            <div className="mt-7 overflow-hidden rounded-[20px] border border-[#dfe3da] bg-white">
              ...
              <div className="divide-y divide-[#e8ebe5]">
                {pages.map((page) => {
                  const liveUrl = publicPageUrl(page.id);
                  return (
                    <article key={page.id} className="grid gap-5 p-5 md:grid-cols-[minmax(0,1fr)_190px_220px] md:items-center md:px-6">
                      <div className="min-w-0">
                        <div className="flex items-center gap-3">
                          <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#eef5e8] text-[#567800]"><Globe2 size={18} /></span>
                          <div className="min-w-0">
                            <h2 className="truncate font-semibold text-[#151914]">{page.id}</h2>
                            <p className="mt-1 truncate font-mono text-[11px] text-[#8a9186]">{publicPageAddress(page.id)}</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2 md:justify-end">
                        <a href={liveUrl} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl border border-[#d4d9d0] px-4 text-sm font-semibold text-[#151914] hover:bg-[#f7f8f4] md:flex-none">
                          View <ArrowUpRight size={15} />
                        </a>
                        <Link href={`/pages/${encodeURIComponent(page.id)}/edit`} className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-[#151914] px-4 text-sm font-semibold text-white hover:bg-[#293025] md:flex-none">
                          Edit <Edit3 size={15} />
                        </Link>
                      </div>
                    </article>
