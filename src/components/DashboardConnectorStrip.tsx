...
export function DashboardConnectorStrip() {
  ...
  return (
    <section className="flex flex-col gap-4 rounded-[4px] border border-[#dfe3da] bg-white p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5" aria-label="Jobing AI connector">
      ...
      <div className="flex shrink-0 gap-2">
        <button type="button" onClick={copyUrl} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-[4px] border border-[#cfd4cb] px-4 text-sm font-semibold text-[#151914] hover:bg-[#f7f8f4] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#719500]" aria-label={copied ? "Connector URL copied" : "Copy connector URL"}>
          {copied ? <Check size={16} /> : <Copy size={16} />}
          {copied ? "Copied" : "Copy"}
        </button>
        <Link href="/connector" style={{ color: "#fff" }} className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-[4px] bg-[#151914] px-4 text-sm font-semibold hover:bg-[#293025] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#719500] sm:flex-none">
