    ...
    <header className="sticky top-0 z-50 border-b border-[#e1e4dc] bg-[#f7f8f4]/95 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6">
        ...
        <nav className="hidden items-center gap-7 text-sm font-semibold text-[#656d62] md:flex" aria-label="Main navigation">
          ...
          <a href={DOCS_URL} className="hover:text-[#151914]">Docs</a>
      ...
      {open ? <nav className="grid gap-1 border-t border-[#e1e4dc] bg-[#f7f8f4] p-3 text-sm font-semibold text-[#555d52] md:hidden" aria-label="Mobile navigation"><Link className="min-h-11 rounded-lg px-3 py-3" href="/#connect" onClick={() => setOpen(false)}>How to connect</Link><Link className="min-h-11 rounded-lg px-3 py-3" href="/#use-cases" onClick={() => setOpen(false)}>What it can do</Link><Link className="min-h-11 rounded-lg px-3 py-3" href="/blog" onClick={() => setOpen(false)}>Guides</Link><a className="min-h-11 rounded-lg px-3 py-3" href={DOCS_URL} onClick={() => setOpen(false)}>Docs</a><Link className="min-h-11 rounded-lg px-3 py-3" href="/pricing" onClick={() => setOpen(false)}>Pricing</Link><Link className="dark-surface-link min-h-11 rounded-lg bg-[#151914] px-3 py-3 text-center" href="/dashboard" prefetch={false} onClick={() => setOpen(false)}>Dashboard</Link></nav> : null}
    </header>
  );
}
