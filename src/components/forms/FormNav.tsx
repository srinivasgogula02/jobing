...
export function FormNav({ formId, current }: { formId: string; current: "build" | "responses" | "share" | "integrations" }) {
  ...
  return <nav className="form-nav" aria-label="Form sections">
    ...
    <Link href={`${base}/integrations`} aria-current={current === "integrations" ? "page" : undefined}>Integrations</Link>
  </nav>;
}
